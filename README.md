...
Audio Sentiment Bot is now a zero-backend, browser-first experience. The app runs entirely on the client using WebAssembly-powered models to transcribe spoken English and predict sentiment, all without shipping audio to a server.
...
3. Drop a short audio clip (≤ ~90 seconds). The first run downloads model weights and caches them locally; subsequent runs are fast and offline-friendly.
...
- `docs/assets/styles.css` – Responsive layout and component styling.
- `docs/assets/app.js` – Client-side waveform rendering, audio resampling, speech transcription, and sentiment scoring using `@xenova/transformers`.
...
- Microphones are optional; the app works with recorded files (`.wav`, `.mp3`, `.m4a`, `.aac`, etc.).
